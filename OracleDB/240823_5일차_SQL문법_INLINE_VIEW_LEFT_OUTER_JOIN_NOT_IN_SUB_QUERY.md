...
# Pagination Query
* 대량의 데이터셋을 작은 페이지로 나눠 효율적으로 관리하고 조회할 수 있는 기법을 의미한다. 예를 들어, 데이터베이스에서 수백만 개의 컬럼을 한꺼번에 가져오는 것은 비효율적이고 성능 문제를 야기할 수 있다. 따라서, 데이터를 특정 페이지 크기로 나눠 필요한 부분만 가져오는 방식을 사용하는 Pagination Query 이용합니다. 
